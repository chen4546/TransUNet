...
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_current.append(loss.__float__())
        loss_avg = sum(loss_current) / len(loss_current)
        loss_epoch = {
            "epoch":epoch_num+1,
            "loss": loss_current
        }
        ...
        with open(loss_file, "a", encoding='utf-8') as f:
            f.write(loss_epoch.__str__())
            f.write('\r')

        logging.info("epoch: {} , loss_avg: {}".format(epoch_num + 1, loss_avg))
        ######
        if (epoch_num + 1) % 5 == 0:
